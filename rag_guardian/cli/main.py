"""``rag-guardian`` command-line entry point.

Commands:

* ``scan``      -- scan a file and write redacted output or a JSON report.
* ``stream``    -- filter a live stdin stream (``tail -f app.log | rag-guardian stream``).
...
from rag_guardian.config.settings import get_settings
from rag_guardian.core.container import Container
from rag_guardian.observability.logging_config import configure_logging
...
@click.version_option(__version__, prog_name="rag-guardian")
...
def cli(log_level: str, log_format: str) -> None:
    """RAG Guardian -- pluggable AI security gateway infrastructure."""
...
def scan(
    ...
    from rag_guardian.ingestion.factory import SourceType
...
def serve(host: str | None, port: int | None, reload: bool) -> None:
    ...
    try:
        ...
        raise click.ClickException(
            "uvicorn is not installed; install with: pip install 'rag-guardian[api]'"
        ) from exc
    settings = get_settings()
    uvicorn.run(
        "rag_guardian.api.app:create_app",
        factory=True,
